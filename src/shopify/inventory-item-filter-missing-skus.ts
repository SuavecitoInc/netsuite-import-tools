...
const DEFAULT_INPUT_FILENAME = 'MISSING_SKUS_SHOPIFY_ROWS'; // SHOPIFY-ITEMS-EXPORT
...
async function main() {
  try {
    ...
    const shopifyExportFilePath = `output/${inputFilename}`;
    ...
    // load missing shopify skus from csv file to use to filter
    const missingSkusFilePath = 'output/missing_shopify_skus';
    const missingSkusFileRows = await parseCSV<{
      Handle: string;
      SKU: string;
    }>(missingSkusFilePath);
    const missingSkusSet = new Set(
      missingSkusFileRows.map((row) => row['SKU'].trim()),
    );
    const missingSkusIndexingDoneAt = Date.now();
    ...
    const netSuiteImportItems = filteredShopifyItemRows
      .map((item, index) => {
        if (!missingSkusSet.has(item['Variant SKU'].trim())) {
          return null; // skip items that are not in the missing SKUs list
        }
    ...
    // export file name
    const outputFilename = 'Shopify_to_NetSuite_Missing_Inventory_Items';
